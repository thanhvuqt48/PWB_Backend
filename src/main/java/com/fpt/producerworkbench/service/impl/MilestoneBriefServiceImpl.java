...
public class MilestoneBriefServiceImpl implements MilestoneBriefService {
    ...
    private static final Set<String> IMAGE_MIMES = Set.of(MediaType.IMAGE_JPEG_VALUE, MediaType.IMAGE_PNG_VALUE,
            "image/webp", "image/gif");
    private static final Set<String> AUDIO_MIMES = Set.of("audio/mpeg", "audio/wav", "audio/x-wav", "audio/mp4",
    ...
    @Override
    public List<MilestoneBriefGroupResponse> createBriefGroups(Long projectId, Long milestoneId,
            MilestoneBriefUpsertRequest request, Authentication auth) {
        ...
        try {
            ...
            if (isClient && project.getCreator() != null) {
                sendBriefNotificationAndEmail(
                        project.getCreator(),
                        currentUser,
                        project,
                        milestone,
                        "Bản mô tả mới đã được tạo",
                        String.format("%s đã tạo bản mô tả mới cho cột mốc \"%s\" trong dự án \"%s\".",
                                currentUser.getFullName() != null ? currentUser.getFullName() : currentUser.getEmail(),
                                milestone.getTitle(),
                                project.getTitle()));
            } else if (!isClient && project.getClient() != null) {
                sendBriefNotificationAndEmail(
                        project.getClient(),
                        currentUser,
                        project,
            ...
            log.error("Gặp lỗi khi gửi thông báo cho createBriefGroups: {}", e.getMessage());
        }
    ...
    @Transactional
    public List<Long> doSaveBriefGroups(Long projectId, Long milestoneId, MilestoneBriefUpsertRequest request,
            Authentication auth) {
        Milestone milestone = loadMilestoneAndCheckAuth(projectId, milestoneId, auth);
    ...
    public MilestoneBriefDetailResponse upsertMilestoneBrief(Long projectId, Long milestoneId,
        ...
        List<MilestoneBriefGroup> currentGroups = briefGroupRepository
                .findByMilestoneIdAndScopeOrderByPositionAsc(milestoneId, MilestoneBriefScope.EXTERNAL);

        // 2. Delete ALL existing blocks and groups first (clean slate approach)
        ...
        try {
            ...
            if (isClient && project.getCreator() != null) {
                sendBriefNotificationAndEmail(
                        ...
                        project,
                        milestone,
                        "Bản mô tả đã được cập nhật",
                        String.format("%s đã cập nhật bản mô tả cho cột mốc \"%s\" trong dự án \"%s\".",
                                currentUser.getFullName() != null ? currentUser.getFullName() : currentUser.getEmail(),
                                milestone.getTitle(),
                                project.getTitle()));
            } else if (!isClient && project.getClient() != null) {
            ...
            log.error("Gặp lỗi khi gửi thông báo cho upsertMilestoneBrief: {}", e.getMessage());
        }
    ...
    public MilestoneBriefDetailResponse upsertInternalMilestoneBrief(Long projectId, Long milestoneId,
        ...
        List<MilestoneBriefGroup> existingGroups = briefGroupRepository
                .findByMilestoneIdAndScopeOrderByPositionAsc(milestoneId, MilestoneBriefScope.INTERNAL);
        ...
        // Send notifications to collaborators and observers
        try {
            Project project = milestone.getContract().getProject();
            User currentUser = userRepository.findByEmail(auth.getName())
                    .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_FOUND));

            sendBriefNotificationToMilestoneMembers(
                    currentUser,
                    project,
                    milestone,
                    "Bản mô tả nội bộ đã được cập nhật",
                    String.format("%s đã cập nhật bản mô tả nội bộ cho cột mốc \"%s\" trong dự án \"%s\".",
                            currentUser.getFullName() != null ? currentUser.getFullName() : currentUser.getEmail(),
                            milestone.getTitle(),
                            project.getTitle()));
        } catch (Exception e) {
            log.error("Gặp lỗi khi gửi thông báo cho upsertInternalMilestoneBrief: {}", e.getMessage());
        }
    ...
    @Override
    @Transactional
    public MilestoneBriefGroupResponse forwardExternalGroupToInternal(Long projectId, Long milestoneId, Long groupId,
            Authentication auth) {
        log.info("Forwarding/Syncing Group ID {} from EXTERNAL to INTERNAL", groupId);
        Milestone milestone = loadMilestoneAndCheckAuth(projectId, milestoneId, auth);
        validateOwnerOnly(auth, projectId);

        MilestoneBriefGroup sourceGroup = findGroupAndValidateBelonging(groupId, milestoneId,
                MilestoneBriefScope.EXTERNAL);

        // Tìm bản sao cũ trong Internal
        Optional<MilestoneBriefGroup> existingGroupOpt = briefGroupRepository.findByForwardIdAndScope(groupId,
        ...
        // Send notifications to collaborators, observers, and client
        try {
            Project project = milestone.getContract().getProject();
            User currentUser = userRepository.findByEmail(auth.getName())
                    .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_FOUND));

            // Notify milestone members (collaborators and observers)
            sendBriefNotificationToMilestoneMembers(
                    currentUser,
                    project,
                    milestone,
                    "Bản mô tả đã được chuyển tiếp",
                    String.format(
                            "%s đã chuyển tiếp bản mô tả \"%s\" từ ngoại bộ sang nội bộ cho cột mốc \"%s\" trong dự án \"%s\".",
                            currentUser.getFullName() != null ? currentUser.getFullName() : currentUser.getEmail(),
            ...
            log.error("Gặp lỗi khi gửi thông báo cho forwardExternalGroupToInternal: {}", e.getMessage());
        }
    ...
    @Override
    @Transactional
    public String uploadBriefFile(Long projectId, Long milestoneId, MultipartFile file, String type,
            Authentication auth) {
        Milestone milestone = loadMilestoneAndCheckAuth(projectId, milestoneId, auth);
        validateExternalWritePermission(auth, projectId);
        ...
        String key = folder + "/" + projectId + "_" + milestoneId + "_" + System.currentTimeMillis() + "_"
    ...
    private void validateInternalReadPermission(Authentication auth, Long projectId, Long milestoneId) {
        ...
        if (roleInfo.userRole == UserRole.PRODUCER && roleInfo.projectRole == ProjectRole.OWNER)
            return;
        ...
        if (roleInfo.projectRole == ProjectRole.COLLABORATOR || roleInfo.projectRole == ProjectRole.OBSERVER) {
            ...
            if (!isMember)
                throw new AppException(ErrorCode.ACCESS_DENIED, "Bạn chưa được thêm vào cột mốc này");
            return;
        }
    ...
    private Milestone loadMilestoneAndCheckAuth(Long projectId, Long milestoneId, Authentication auth) {
        ...
        Milestone milestone = milestoneRepository.findById(milestoneId)
                .orElseThrow(() -> new AppException(ErrorCode.MILESTONE_NOT_FOUND));
        if (!milestone.getContract().getProject().getId().equals(projectId))
            throw new AppException(ErrorCode.ACCESS_DENIED);
        return milestone;
    }
    ...
    private MilestoneBriefGroup findGroupAndValidateBelonging(Long groupId, Long milestoneId,
            MilestoneBriefScope scope) {
        MilestoneBriefGroup group = briefGroupRepository.findById(groupId)
                .orElseThrow(() -> new AppException(ErrorCode.RESOURCE_NOT_FOUND));
        if (!group.getMilestone().getId().equals(milestoneId) || group.getScope() != scope)
            throw new AppException(ErrorCode.INVALID_PARAMETER_FORMAT);
        return group;
    }

    private MilestoneBriefGroup createGroupEntity(Milestone milestone, MilestoneBriefGroupRequest req,
            MilestoneBriefScope scope) {
        MilestoneBriefGroup group = MilestoneBriefGroup.builder()
                .milestone(milestone)
                .scope(scope)
    ...
    private List<MilestoneBriefBlock> mapBlocksRequestToEntity(List<MilestoneBriefBlockRequest> blockReqs,
            MilestoneBriefGroup group) {
        List<MilestoneBriefBlock> blocks = new ArrayList<>();
        int index = 1;
        for (MilestoneBriefBlockRequest br : blockReqs) {
            String fileKey = (br.getType() == MilestoneBriefBlockType.IMAGE
                    || br.getType() == MilestoneBriefBlockType.HUM_MELODY) ? br.getContent() : null;
            String text = (fileKey == null) ? br.getContent() : null;
            String label = (br.getLabel() != null && !br.getLabel().isBlank()) ? br.getLabel()
                    : defaultLabelByType(br.getType());
            blocks.add(MilestoneBriefBlock.builder().group(group).type(br.getType()).label(label).contentText(text)
                    .fileKey(fileKey).position(br.getPosition() != null ? br.getPosition() : index++).build());
        }
        return blocks;
    }

    private MilestoneBriefDetailResponse buildDetailResponse(Milestone milestone, MilestoneBriefScope scope) {
        List<MilestoneBriefGroup> groups = briefGroupRepository
                .findByMilestoneIdAndScopeOrderByPositionAsc(milestone.getId(), scope);
        return MilestoneBriefDetailResponse.builder().milestoneId(milestone.getId())
                .projectId(milestone.getContract().getProject().getId()).scope(scope)
                .groups(groups.stream().map(this::mapToGroupResponse).collect(Collectors.toList())).build();
    }
    ...
    private MilestoneBriefGroupResponse mapToGroupResponse(MilestoneBriefGroup group) {
        List<MilestoneBriefBlockResponse> blockResponses = group.getBlocks() == null ? Collections.emptyList()
                : group.getBlocks().stream()
                        .sorted(Comparator.comparing(MilestoneBriefBlock::getPosition,
                                Comparator.nullsLast(Integer::compareTo)))
                        .map(this::mapToBlockResponse).collect(Collectors.toList());
        return MilestoneBriefGroupResponse.builder().id(group.getId()).title(group.getTitle())
                .position(group.getPosition()).blocks(blockResponses).build();
    }
    ...
    private MilestoneBriefBlockResponse mapToBlockResponse(MilestoneBriefBlock block) {
        ...
        return MilestoneBriefBlockResponse.builder().id(block.getId()).type(block.getType()).label(block.getLabel())
                .content(content).position(block.getPosition()).build();
    }
    ...
    private void sendBriefNotificationAndEmail(User recipient, User sender, Project project, Milestone milestone,
            String action, String message) {
        try {
            String actionUrl = String.format("/project-workspace?milestoneId=%d", milestone.getId());
        ...
        try {
            ...
            params.put("senderName", sender.getFullName() != null ? sender.getFullName() : sender.getEmail());
            ...
            NotificationEvent event = NotificationEvent.builder()
                    ...
                    .subject(action + ": " + milestone.getTitle())
                    .templateCode("milestone-brief-update-template")
                    .param(params)
                    .build();
            kafkaTemplate.send(NOTIFICATION_TOPIC, event);
            log.info("Đã gửi email thông báo cập nhật bản mô tả thành công!");
        } catch (Exception e) {
            log.error("Gặp lỗi khi gửi email thông báo cập nhật bản mô tả: {}", e.getMessage());
        }
    }

    private void sendBriefNotificationToMilestoneMembers(User sender, Project project, Milestone milestone,
            String action, String message) {
        List<MilestoneMember> members = milestoneMemberRepository.findByMilestoneId(milestone.getId());

        for (MilestoneMember member : members) {
            User recipient = member.getUser();
            if (recipient != null && !recipient.getId().equals(sender.getId())) {
                sendBriefNotificationAndEmail(recipient, sender, project, milestone, action, message);
            }
        }
    }
    ...
    private static class RoleInfo {
        ...
        RoleInfo(UserRole u, ProjectRole p) {
            this.userRole = u;
            this.projectRole = p;
        }
    }
