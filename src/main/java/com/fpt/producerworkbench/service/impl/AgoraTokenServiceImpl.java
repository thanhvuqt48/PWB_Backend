...
public class AgoraTokenServiceImpl implements AgoraTokenService {
    ...
    public String generateToken(String channelName) {
        return generateRtcToken(channelName, 0, Role.Role_Publisher, 24 * 3600);
    }

    @Override
    public String generateRtcToken(String channelName, int uid, Role role, int expirationTimeInSeconds) {
        // Validate inputs
    ...
    private void validateChannel(String channelName) {
        if (channelName == null || channelName.trim().isEmpty()) {
            throw new AgoraTokenException(ErrorCode.AGORA_CHANNEL_INVALID, "Channel name cannot be empty");
        }

        if (channelName.length() > 64) {
            throw new AgoraTokenException(ErrorCode.AGORA_CHANNEL_INVALID, "Channel name cannot exceed 64 characters");
        }
    }

    private void validateUid(int uid) {
        if (uid < 0) {
            throw new AgoraTokenException(ErrorCode.AGORA_UID_INVALID, "UID cannot be negative");
        }
    }
    ...
    private void validateConfig() {
        ...
        if (appId == null || appId.trim().isEmpty()) {
            throw new AgoraTokenException(ErrorCode.AGORA_CONFIG_MISSING, "Agora App ID is not configured");
        }

        if (appCert == null || appCert.trim().isEmpty()) {
            throw new AgoraTokenException(ErrorCode.AGORA_CONFIG_MISSING, "Agora App Certificate is not configured");
        }
    }
}
