...
//        if (payMilestone) validateMilestonesAmount(req.getMilestones(), totals.grandTotal);
...
//
//            com.itextpdf.layout.element.Div probe = buildMilestoneBlock(font, rect.getWidth(), title, due, money, desc);
...
//            }
//
//            com.itextpdf.layout.element.Div real = buildMilestoneBlock(font, rect.getWidth(), title, due, money, desc);
//            canvas.add(real);
//            remaining -= needed;
//        }
//
...
public class ContractPdfServiceImpl implements ContractPdfService {
    ...
    public byte[] fillTemplate(Authentication auth, ContractPdfFillRequest req) {
        ...
        // ==== Validate theo phương án thanh toán ====
        if (payOnce) {
            // Yêu cầu #2: Bắt buộc nhập khi thanh toán Full
            if (req.getFpEditAmount() == null) throw new AppException(ErrorCode.BAD_REQUEST);
        }
        if (payMilestone) {
            // Yêu cầu #3: milestones must not be empty & mỗi item phải có editCount
        ...
        // Lưu FP Edit Amount nếu có (yêu cầu #2)
        ...
        if (payMilestone && req.getMilestones() != null) {
            ...
            for (MilestoneRequest m : req.getMilestones()) {
                Milestone ms = Milestone.builder()
                        ...
                        .status(MilestoneStatus.PENDING)
                        .sequence(idx)
                        // Lưu số lần chỉnh sửa trên từng cột mốc (yêu cầu #3)
        ...
        try (InputStream in = templateResource.getInputStream();
            ...
            // ==== Yêu cầu #1: map field "Percent" ====
            fields.put("Percent", ns(req.getPercent()));

            // ==== Yêu cầu #2: nếu có field "FP Edit Amount" trên PDF thì fill ====
            if (req.getFpEditAmount() != null) {
                fields.put("FP Edit Amount", String.valueOf(req.getFpEditAmount()));
            } else {
                // nếu null mà payOnce thì đã validate ở trên (throw BAD_REQUEST), còn payMilestone thì có thể để trống
