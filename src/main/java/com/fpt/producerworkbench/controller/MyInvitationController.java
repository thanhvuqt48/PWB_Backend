...
public class MyInvitationController {
    ...
    public ResponseEntity<ApiResponse<List<InvitationResponse>>> getMyInvitations(@AuthenticationPrincipal Jwt jwt) {
        User currentUser = userRepository.findByEmail(jwt.getSubject()).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_FOUND));
        List<InvitationResponse> invitations = invitationService.getMyPendingInvitations(currentUser);
